...
#   apt update && apt-get --no-install-recommends install -y libssl-dev ca-certificates wget gnupg2 lsb-release pkg-config systemd
#
...
# RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
#   --mount=type=cache,target=/var/lib/apt,sharing=locked \
#   apt update && apt-get --no-install-recommends install -y tor deb.torproject.org-keyring elasticsearch
